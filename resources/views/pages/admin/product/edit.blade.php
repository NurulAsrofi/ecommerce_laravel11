...
@section('content')
    <div class="main-content">
        <section class="section">
            ...
            <div class="card mt-4">
                <form action="{{ route('admin.product.update', $product->id) }}" class="needs-validation" novalidate=""
                                    ...
                                    <select name="id_distributor" class="form-control">
                                        @foreach ($distributor as $item)
                                            <option value="{{ $item->id }}"
                                                {{ $product->id_distributor == $item->id ? 'selected' : '' }}>
                            ...
                            <div class="col-6">
                                <div class="form-group">
                                    ...
                                    <input id="discount" type="number" class="form-control" name="discount"
                                        placeholder="Diskon (%)" value="{{ $product->discount ?? 0 }}" required>
                                    <div class="invalid-feedback">Kolom ini harus di isi!</div>
                                </div>
                            </div>
                            <div class="col-6">
